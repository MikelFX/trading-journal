...
export default async function DashboardPage() {
  ...
  return (
    <PageWrapper>
      ...
      {/* KPI grid */}
      <div className="kpi-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: 16, marginBottom: 32 }}>
      ...
      {stats.equityCurve.length > 1 && (
        <FadeUp delay={0.3}>
          <div style={{
            ...
            <div style={{ padding: "14px 24px", borderBottom: "1px solid var(--color-border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h2 style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--color-text-muted)" }}>
                Equity křivka
              </h2>
              <Link href="/analytics" style={{ fontSize: 12, color: "var(--color-accent)", textDecoration: "none" }}>
                Analytika →
              </Link>
            </div>
            <div style={{ padding: "16px 16px 8px" }}>
              <EquityChart data={stats.equityCurve} height={180} />
            </div>
          </div>
        </FadeUp>
      )}
      ...
      {/* Recent trades */}
      <FadeUp delay={stats.equityCurve.length > 1 ? 0.4 : 0.3}>
        <div style={{
          background: "var(--color-surface)",
          border: "1px solid var(--color-border)",
          borderRadius: "var(--radius-lg)",
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 12 }}>
              <Link href="/trades/import" style={{ fontSize: 12, color: "var(--color-text-muted)", textDecoration: "none" }}>
                CSV import
              </Link>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 500 }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid var(--color-border)" }}>
                      {["Symbol", "Směr", "Setup", "Vstup", "R", "P/L", "AI"].map((h) => (
                  ...
                  <tbody>
                    {recent.map((t: TradeWithRelations) => (
                      <StaggerItem key={t.id}>
                        <tr className="tr-hover" style={{ borderBottom: "1px solid rgba(26,36,56,0.6)", display: "table-row" }}>
                          <td style={{ padding: "13px 20px", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 13 }}>
                            <Link href={`/trades/${t.id}`} style={{ color: "var(--color-text)", textDecoration: "none" }}>{t.symbol}</Link>
                          </td>
                          <td style={{ padding: "13px 20px" }}>
                            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", color: t.direction === "LONG" ? "var(--color-profit)" : "var(--color-loss)", background: t.direction === "LONG" ? "var(--color-profit-dim)" : "var(--color-loss-dim)", padding: "3px 8px", borderRadius: 4 }}>
                              {t.direction}
                            </span>
                          </td>
                          <td style={{ padding: "13px 20px", color: "var(--color-text-muted)", fontSize: 13 }}>{t.setup?.name ?? "—"}</td>
                          ...
                          <td className="tabular" style={{ padding: "13px 20px", fontWeight: 600, color: t.realizedPnl !== null ? (t.realizedPnl >= 0 ? "var(--color-profit)" : "var(--color-loss)") : "var(--color-text-muted)" }}>
                            {t.realizedPnl !== null ? `${t.realizedPnl >= 0 ? "+" : ""}${t.realizedPnl.toLocaleString("cs-CZ", { minimumFractionDigits: 2 })} ${currency}` : "—"}
                          </td>
                          <td style={{ padding: "13px 20px" }}>
                            {t.aiNote ? (
                              <span title={t.aiNote} style={{ fontSize: 11, color: "var(--color-accent)", cursor: "help" }}>✦ AI</span>
                            ) : t.status === "CLOSED" ? (
